...
log "Building container (dev-text2video)..."
(
  cd "$ROOT_DIR"
  if [[ "$NO_CACHE" == "1" ]]; then
    docker compose build --no-cache dev-text2video
  else
    docker compose build dev-text2video
  fi
)

log "Starting container (dev-text2video)..."
(
  cd "$ROOT_DIR"
...
if [[ -f "$MISSING_REQ" ]]; then
  ...
  (
    cd "$ROOT_DIR"
    docker compose run --rm dev-text2video bash -lc "pip install -r /workspace/scripts/missing-deps.txt"
  )
fi
